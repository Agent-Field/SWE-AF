...
**QA synthesizer fix (2026-08-05, issue #113):** `router.ai(..., schema=...)` in AgentField SDK now returns the Pydantic model directly (no legacy `.parsed` wrapper). Fixed in `swe_af/reasoners/execution_agents.py` with `getattr(result, "parsed", result)` to handle both old and new SDK versions. Fast build smoke tests now pass the verify stage including QA synthesis.

**Build timeout raised (2026-08-05):** `FastBuildConfig.build_timeout_seconds` default increased from 600 to 1800 seconds (commit `2755a1e`). 3–6 task pipelines on 9Router+OpenCode require ~955 s for coding alone; 1800 s provides headroom for full plan→execute→verify cycles.
