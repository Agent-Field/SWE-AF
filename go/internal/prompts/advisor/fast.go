...
// FastPlannerTaskOptions mirrors the keyword arguments of the Python
// fast_planner_task_prompt.
type FastPlannerTaskOptions struct {
	Goal              string
	RepoPath          string
	MaxTasks          int
...
// FastPlannerTaskPrompt ports swe_af.fast.prompts.fast_planner_task_prompt.
func FastPlannerTaskPrompt(opts FastPlannerTaskOptions) string {
	contextBlock := ""
	...
	return "## Goal\n" + opts.Goal + "\n\n## Repository\n" + opts.RepoPath + "\n" + contextBlock + "\n## Constraints\n\n" +
		"- Produce at most " + strconv.Itoa(opts.MaxTasks) + " tasks.\n" +
...
const FastPlannerSystemPrompt = `You are a senior software architect specializing in rapid, single-pass delivery.
...
- Each task ` +
	...
	`name` +
	"`" +
	` must be a unique kebab-case slug (lowercase, hyphens only).
- ` +
	"`" +
	`acceptance_criteria` +
	"`" +
...
- ` +
	...
	`files_to_modify` +
	"`" +
	` default to empty lists if unused.
- ` +
	"`" +
	`estimated_minutes` +
	"`" +
	` is a positive integer estimate per task.`
