...
// SprintPlannerSystemPrompt is the verbatim SYSTEM_PROMPT from sprint_planner.py.
const SprintPlannerSystemPrompt = `You are a senior Engineering Manager who has run dozens of autonomous agent teams.
...
// sprintPlannerMissionBody is the static (backtick-heavy) tail of the
// sprint_planner_prompts task template, from "## Your Mission" onward.
const sprintPlannerMissionBody = `## Your Mission
...
// SprintPlannerPromptsOpts carries the keyword-only params of sprint_planner_prompts.
type SprintPlannerPromptsOpts struct {
	PRD              schemas.PRD
...
// SprintPlannerPrompts ports sprint_planner_prompts: returns (system, task).
func SprintPlannerPrompts(o SprintPlannerPromptsOpts) (systemPrompt, task string) {
...
`, o.PRD.ValidatedDescription, acFormatted, o.Architecture.Summary, o.PRDPath, o.ArchitecturePath, o.RepoPath) + sprintPlannerMissionBody
	return SprintPlannerSystemPrompt, task
}

// SprintPlannerTaskPromptOpts carries the keyword-only params of
// sprint_planner_task_prompt. PRD and Architecture are the model_dump() dicts
// (the Python `dict | PRD` union is served by the dict path here).
type SprintPlannerTaskPromptOpts struct {
	Goal              string
	PRD               map[string]any
...
// SprintPlannerTaskPrompt ports sprint_planner_task_prompt (section builder,
// joined with blank lines).
func SprintPlannerTaskPrompt(o SprintPlannerTaskPromptOpts) string {
	var sections []string
	...
	acList := mapStringSlice(o.PRD, "acceptance_criteria")
	...
	archSummary := mapString(o.Architecture, "summary")
	if archSummary != "" {
		sections = append(sections, "## Architecture Summary\n"+archSummary)
	}

	if o.RepoPath != "" || o.PRDPath != "" || o.ArchitecturePath != "" {
		refLines := []string{"## Reference Documents"}
		if o.PRDPath != "" {
			refLines = append(refLines, "- Full PRD: "+o.PRDPath)
		}
		if o.ArchitecturePath != "" {
			refLines = append(refLines, "- Architecture: "+o.ArchitecturePath)
		}
